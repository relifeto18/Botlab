...
, initialPose_(initialPose)
{
    ...
    lcm_subscriptions_.push_back(lcm_.subscribe(LOCALIZATION_ONLY_CHANNEL, &OccupancyGridSLAM::handleLocalizationOnly, this));
...
void OccupancyGridSLAM::handleLocalizationOnly(const lcm::ReceiveBuffer *rbuf, const std::string &channel, const mbot_lcm_msgs::pose_xyt_t *localizationOnly)
{
    std::lock_guard<std::mutex> autoLock(dataMutex_);
    if (localizationOnly->x == 1000) {
        LocalizationOnly_ = true;
    }
