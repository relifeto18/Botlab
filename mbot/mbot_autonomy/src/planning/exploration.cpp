...
void Exploration::executeStateMachine(void)
{
    ...
    if(!executingPlan_ && !are_equal(previousPath_.path, currentPath_.path))
    { 
        printf("LENGTHS= %d, %d\n", previousPath_.path_length, currentPath_.path_length);
...
int8_t Exploration::executeExploringMap(bool initialize)
{
    ...
    */
    ...
    if (executingPlan_ && currentPath_.path_length > 1) {
        currentTarget_ = currentPath_.path[currentPath_.path_length - 1];
        float dist = 0;//std::numeric_limits<float>::max();
        if (currentTarget_.x != 0 || currentTarget_.y != 0)
        {
            dist = std::sqrt(std::pow(currentPose_.x - currentTarget_.x, 2) + std::pow(currentPose_.y - currentTarget_.y, 2));
        }
        if (dist < 0.3)
        {
            executingPlan_ = false;
        }
    }
    ...
    {
        std::cout << "COMPLETED EXPLORATION" << std::endl;
        status.status = mbot_lcm_msgs::exploration_status_t::STATUS_COMPLETE;
    }
...
int8_t Exploration::executeReturningHome(bool initialize)
{
    ...
    */
    ...
    if (!returning_home) {
        ...
        mbot_lcm_msgs::pose_xyt_t localizationOnly;
        localizationOnly.x = 1000;
        lcmInstance_->publish(LOCALIZATION_ONLY_CHANNEL, &localizationOnly);


        executingPlan_ = false;
        bool valid = nearest_navigable_cell(currentPose_, Point<float>(homePose_.x, homePose_.y), currentPath_, currentMap_, planner_);
        currentPath_.path_length = currentPath_.path.size();
        currentPath_.path[currentPath_.path_length - 1].theta = 0;
