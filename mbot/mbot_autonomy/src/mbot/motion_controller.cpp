...
public:
    ...
    virtual mbot_lcm_msgs::mbot_motor_command_t get_command(const mbot_lcm_msgs::pose_xyt_t& pose, const mbot_lcm_msgs::pose_xyt_t& target) override
    {
        ...
        //std::cout << "THETA ERROR = " << d_theta << std::endl;
    ...
    {
        bool reached = ((fabs(pose.x - target.x) < 0.02) && (fabs(pose.y - target.y)  < 0.02));
        if (reached) {
            fwd_sum_error = 0;
            fwd_last_error = 0;
            turn_sum_error = 0;
...
public:
    ...
    virtual bool target_reached(const mbot_lcm_msgs::pose_xyt_t& pose, const mbot_lcm_msgs::pose_xyt_t& target, bool is_end_pose)  override
    {
        ...
        bool reached =  (fabs(angle_diff(pose.theta, target_heading)) < 0.05);
    ...
    bool target_reached_final_turn(const mbot_lcm_msgs::pose_xyt_t& pose, const mbot_lcm_msgs::pose_xyt_t& target)
    {
        ...
        float target_heading = atan2(dy, dx);
        // Handle the case when the target is on the same x,y but on a different theta
        bool reached = (fabs(angle_diff(target.theta, pose.theta)) < 0.05);
...
private:
    ...
    enum State
    {
        INITIAL_TURN=0,
        DRIVE=1,
        FINAL_TURN=2, // to get to the pose heading
        SMART=3
    };
...
int main(int argc, char** argv)
{
    ...
    while(true)
    {
    	...
    	if(controller.timesync_initialized()){
            ...
            float max_forward_vel = 0.25;
            if (cmd.trans_v > max_forward_vel) cmd.trans_v = max_forward_vel;
            else if (cmd.trans_v < -max_forward_vel) cmd.trans_v = -max_forward_vel;

            // Angular vel
            float max_ang_vel = M_PI / 8.0;
